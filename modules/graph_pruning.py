...
class GraphPruning(nn.Module):
    def __init__(
        ...
        self._ent_initial_proj = torch.nn.Linear(
            embedding_dim + encoder_output_dim + 1, embedding_dim
        )
        self.dim = embedding_dim

        self._gnn = GatedGraphConv(
            self.dim, timesteps, num_edge_types=num_edge_types, dropout=rnn_dropout
        )

        self._graph_global_node_init_emb = torch.nn.Parameter(
            torch.FloatTensor(1, embedding_dim)
        )
    ...
    def forward(
        ...
        linking_scores_with_null = torch.cat(
            (linking_scores.new_zeros(*linking_scores.size()[:2], 1), linking_scores),
            dim=-1,
        )
        utterance_mask_with_null = torch.cat(
            (utterance_mask.new_zeros(utterance_mask.size(0), 1), utterance_mask),
            dim=-1,
        )

        # (batch_size, num_entities, num_question_tokens)
        linking_probabilities = util.masked_softmax(
            linking_scores_with_null, utterance_mask_with_null
        )
        ...
        relevance, relevance_logits = self._calculate_relevance(
            worlds, initial_embedding2, get_graph_adj_lists
        )
    ...
    def _calculate_relevance(
        ...
        for batch_index, world in enumerate(worlds):
            ...
            adj_list = get_graph_adj_lists(
                initial_embedding.device,
                world,
                initial_embedding.size(1) - 1,
                global_node=True,
            )
            graph_data = Data(x)
        ...
        gnn_output = self._gnn(
            batch.x, [batch[f"edge_index_{i}"] for i in range(self._gnn.num_edge_types)]
        )
