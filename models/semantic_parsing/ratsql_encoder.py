...
class RatsqlEncoder(SchemaEncoder):
    def __init__(
        ...
        self.rat_encoder = transformer.Encoder(
            lambda: transformer.EncoderLayer(
                ...
                transformer.MultiHeadedAttentionWithRelations(
                    num_heads, hidden_size, 0.1
                ),
                transformer.PositionwiseFeedForward(hidden_size, ff_size, 0.1),
                n_relations,
                0.1,
            ),
            hidden_size,
        ...
        self._att_question_schema = BilinearMatrixAttention(
            self._embedding_dim, self._embedding_dim
        )
        self._att_schema_schema = BilinearMatrixAttention(
            self._embedding_dim, self._embedding_dim
        )
        ...
        self._encoder_output_dim = (
            self._action_embedding_dim + self._action_embedding_dim
        )
        self._to_emb = torch.nn.Linear(self._embedding_dim, self._action_embedding_dim)
        self._to_emb_big = torch.nn.Linear(
            self._embedding_dim, self._encoder_output_dim
        )
    ...
    def _get_initial_state(
        ...
        ) = parser_utils.batched_span_select(embedded_utterance_schema, offsets)
        embedded_utterance_schema = util.masked_mean(
            embedded_utterance_schema,
        ...
        enriched_utterance_schema = self.rat_encoder(
            embedded_utterance_schema, relation.long(), relation_mask
        )
        utterance_schema, utterance_schema_mask = parser_utils.batched_span_select(
        ...
        final_encoder_output = self._to_emb_big(
            util.masked_mean(utterance, utterance_mask.unsqueeze(-1), dim=1)
        )
        ...
        encoder_outputs = self._to_emb_big(utterance)
        memory_cell = torch.zeros(
            [batch_size, self._encoder_output_dim], device=device
        )  # OK

        initial_score = torch.zeros([batch_size], device=device)  # OK
        schema = self._to_emb(schema)
