...
class SpiderParser(Model):
    def __init__(
        ...
        self._transition_function = LinkingTransitionFunction(
            encoder_output_dim=self._schema_encoder._encoder_output_dim,
            action_embedding_dim=self._schema_encoder._action_embedding_dim,
            input_attention=input_attention,
            predict_start_type_separately=False,
            add_action_bias=self._add_action_bias,
    ...
    def forward(
        ...
        initial_state, loss = self._schema_encoder._get_initial_state(
            enc,
            world,
            schema,
            valid_actions,
        ...
        if self.training:
            try:
                ...
                # print("hi")
                return {
                    "loss": Parameter(torch.tensor([0]).float()).to(
            ...
            self._compute_validation_outputs(
                valid_actions, best_final_states, world, action_sequence, outputs
            )
            return outputs
    ...
    def _compute_validation_outputs(
        ...
        for i in range(batch_size):
            ...
            predicted_sql_query = action_sequence_to_sql(
                action_strings, add_table_names=True
            )
            ref_predicted_sql_query = sqlparse.format(
                predicted_sql_query, reindent=False
            )
            ...
            if target_available:
                sequence_in_targets = parser_utils.action_history_match(
                    best_action_indices, targets
                )
                self._exact_match(sequence_in_targets)

                sql_evaluator_match = self._evaluate_func(
                    original_gold_sql_query, predicted_sql_query, world[i].db_id
                )
            ...
            for pos, final_state in enumerate(best_final_states[i]):
                ...
                correct = False
                if target_available:
                    correct = self._evaluate_func(
                        original_gold_sql_query, candidate_sql_query, world[i].db_id
                    )
                    if correct:
                        beam_hit = True
