...
class GnnEncoder(SchemaEncoder):
    def __init__(
        ...
        self._first_action_embedding = torch.nn.Parameter(
            torch.FloatTensor(self._action_embedding_dim)
        )
        ...
        self._graph_pruning = GraphPruning(
            3,
            self._embedding_dim,
        ...
        self._ent2ent_ff = FeedForward(
            action_embedding_dim, 1, action_embedding_dim, Activation.by_name("relu")()
        )
    ...
    def _get_initial_state(
        ...
        max_len_entities = max(
            [len(w.db_context.knowledge_graph.entities) for w in worlds]
        )
        ...
        if action_sequence is not None:
            ...
            for batch_index, batch_actions in enumerate(action_sequence.squeeze(-1)):
                oracle_entities.append(
                    set(
                        [
                            valid_actions[batch_index][action][0]
                            .split(" -> ")[1]
                            .strip('["]')
                            for action in batch_actions
                            if not valid_actions[batch_index][action][1] and action >= 0
                        ]
                    )
                )
            oracle_relevance_score = [
                pad_sequence_to_length(
                    w.get_oracle_relevance_score(oe), max_len_entities
                )
                for w, oe in zip(worlds, oracle_entities)
            ]
            oracle_relevance_score = torch.tensor(
                oracle_relevance_score, dtype=torch.float, device=device
            )
        ...
        entity_types, entity_type_dict = parser_utils.get_type_vector(
        ...
        linking_probabilities = parser_utils.get_linking_probabilities(
            worlds,
        ...
        neighbor_indices = parser_utils.get_neighbor_indices(
            worlds, num_entities, linking_scores.device
        )
        ...
        if self._use_neighbor_similarity_for_linking and neighbor_indices is not None:
            ...
            neighbor_mask = util.get_text_field_mask(
                {"tokens": {"ignored": neighbor_indices + 1}}, num_wrapping_dims=1
            ).float()
        ...
        entities_graph_encoding = parser_utils.get_schema_graph_encoding(
            self._gnn, worlds, graph_initial_embedding
        )
        graph_link_embedding = util.weighted_sum(
            entities_graph_encoding, linking_probabilities
        )
        encoder_outputs = torch.cat((encoder_outputs, graph_link_embedding), dim=-1)
        ...
        final_encoder_output = util.get_final_encoder_states(
            encoder_outputs, utterance_mask.long(), self._encoder.is_bidirectional()
        )
        ...
        initial_grammar_state = [
            self._create_grammar_state(
                ...
                if entities_graph_encoding is not None
                else None,
            )
            for i in range(batch_size)
        ]
        ...
        initial_sql_state = [
            SqlState(actions[i], self._parse_sql_on_decoding) for i in range(batch_size)
        ]
        ...
        if action_sequence is not None:
            ...
            graph_mask = util.get_mask_from_sequence_lengths(
                torch.tensor([len(w.entities_names) for w in worlds], device=device),
                max_len_entities,
            ).float()
            graph_loss = torch.nn.functional.binary_cross_entropy_with_logits(
                ent_relevance_logits.squeeze(-1),
                oracle_relevance_score,
                reduction="none",
            )
    ...
    def _create_grammar_state(
        ...
        for key, action_strings in valid_actions.items():
            ...
            if global_actions:
                ...
                global_action_tensor = (
                    torch.cat(global_action_tensors, dim=0)
                    .to(global_action_tensors[0].device)
                    .long()
                )
        ...
        return GrammarStatelet(
            ["statement"], translated_valid_actions, parser_utils.is_nonterminal
        )
