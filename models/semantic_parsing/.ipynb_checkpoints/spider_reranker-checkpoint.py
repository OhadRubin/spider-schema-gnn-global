...
class SpiderReranker(SpiderBase):
    def __init__(
        ...
        self._q_att_query = torch.nn.Parameter(
            torch.FloatTensor(1, encoder.get_output_dim() + action_embedding_dim)
        )
        torch.nn.init.xavier_uniform_(self._q_att_query)

        self._q_att_proj = torch.nn.Linear(
            encoder.get_output_dim(), action_embedding_dim
        )
        self._linking_params = torch.nn.Linear(16, 1)
        torch.nn.init.uniform_(self._linking_params.weight, 0, 1)

        self._gnn = GatedGraphConv(
            self._embedding_dim, gnn_timesteps, num_edge_types=4, dropout=dropout
        )

        self._graph_global_node_init_emb = torch.nn.Parameter(
            torch.FloatTensor(1, self._embedding_dim)
        )
        ...
        self._ent_initial_proj = torch.nn.Linear(
            initial_embedding_size, self._embedding_dim
        )
        torch.nn.init.xavier_uniform_(self._ent_initial_proj.weight)

        self._final_graph_proj = torch.nn.Linear(
            action_embedding_dim * 2 + encoder.get_output_dim(), self._embedding_dim
        )
        torch.nn.init.xavier_uniform_(self._final_graph_proj.weight)

        self._neighbor_params = torch.nn.Linear(
            self._embedding_dim, self._embedding_dim
        )
    ...
    def forward(
        ...
        candidates_scores, debug_info = self._score_candidates(
            utterance, world, schema, sub_graphs
        )
        ...
        if sub_graphs_labels is not None:
            ...
            if batch_size - num_unanswerable > 0:
                ...
                loss = -torch.log(
                    (normalized_scores * sub_graphs_labels.float()).sum(dim=1)
                )
        ...
        self._compute_validation_outputs(
            world, sub_graphs, candidates_scores, sub_graphs_labels, candidates, outputs
        )
        return outputs
    ...
    def _score_candidates(
        ...
        linking_probabilities = self._get_linking_probabilities(
            worlds, linking_scores.transpose(1, 2), utterance_mask, entity_type_dict
        )
        ...
        q_aligned = util.weighted_sum(
            encoder_outputs, linking_probabilities.transpose(1, 2)
        )

        graph_initial_embedding = torch.cat((entity_type_embeddings, q_aligned), dim=-1)

        # (batch_size, num_entities, graph_embedding)
        graph_initial_embedding_proj = self._dropout(
            self._ent_initial_proj(graph_initial_embedding)
        )

        global_node_initial_embedding = self._graph_global_node_init_emb.repeat(
            batch_size, 1
        )

        graph_initial_embedding_with_global = torch.cat(
            (graph_initial_embedding_proj, global_node_initial_embedding.unsqueeze(1)),
            dim=1,
        )
        ...
        ) = self._get_schema_graph_encoding(
            worlds, graph_initial_embedding_with_global, sub_graphs
        )
        ...
        linking_probabilities = (
            linking_probabilities.unsqueeze(1)
            .expand(-1, num_candidates, -1, -1)
            .contiguous()
            .view(batch_size * num_candidates, num_question_tokens, num_entities)
        )
        link_embedding = util.weighted_sum(
            entities_graph_encoding, linking_probabilities
        )
        ...
        encoder_outputs = (
            ...
            .expand(-1, num_candidates, -1, -1)
            .contiguous()
            .view(batch_size * num_candidates, num_question_tokens, -1)
        )
        ...
        utterance_mask = (
            ...
            .contiguous()
            .view(batch_size * num_candidates, num_question_tokens)
        )

        encoder_augmented_outputs = torch.cat([encoder_outputs, link_embedding], -1)

        q_att_weights = self._attention(
            self._q_att_query.expand(batch_size * num_candidates, -1),
        ...
        encoding_comps = torch.cat((global_schema_encoding, q_aligned), dim=-1)
    ...
    def _get_neighbor_indices(
        ...
        for world in worlds:
            for entity in world.db_context.knowledge_graph.entities:
                if (
                    len(world.db_context.knowledge_graph.neighbors[entity])
                    > num_neighbors
                ):
                    num_neighbors = len(
        ...
        for world in worlds:
            ...
            for entity in entities:
                ...
                padded = pad_sequence_to_length(
                    entity_neighbors, num_neighbors, lambda: -1
                )
    ...
    def _get_schema_graph_encoding(
        ...
        for batch_index, world in enumerate(worlds):
            ...
            for sub_graph in sub_graphs[batch_index]:
                adj_list = self._get_graph_adj_lists(
                    initial_graph_embeddings.device,
                    world,
        ...
        gnn_output = gnn_output.view(
            batch_size * num_candidates, max_num_entities + 1, -1
        )
    ...
    def _get_graph_adj_lists(
        self, device, world, global_entity_id, sub_graph: Set = None
    ):
        entity_mapping = {}
    ...
    def _query_difficulty(query_tokens, entities):
        number_tables = len(
            set([token for token in query_tokens if "table:" + token in entities])
        )
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        return {
            key: metric.get_metric(reset)
            for key, metric in self._accuracy_metrics.items()
        }
    ...
    def _compute_validation_outputs(
        ...
        for i in range(batch_size):
            ...
            sorted_candidates_ids = example_sub_graphs_scores.sort(descending=True)[
                1
            ].tolist()
            sorted_sub_graphs = OrderedSet(
                [
                    sub_graphs_ids[j]
                    for j in sorted_candidates_ids
                ...
                candidates_for_final_sort.append(
                    {
                        ...
                        "reranker_cand_rank": sorted_candidates_ids.index(
                            original_rank
                        ),
            ...
            if sub_graphs_labels is not None and difficulty is not None:
                sg_tsk_query_correct = candidates_sg_sort[0]["correct"]

                self._update_metric(
                    "query_accuracy", int(sg_tsk_query_correct), difficulty
                )
