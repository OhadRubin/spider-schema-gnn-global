...
reader = SpiderRatsqlDatasetReader(question_token_indexers={"tokens": PretrainedTransformerIndexer("bert-base-uncased")},tables_file="dataset/tables.json",cache_directory="cache/train", max_instances=None)
...
schema_encoder = RatsqlEncoder(question_embedder=word_embeddings, action_embedding_dim=768)
...
model = SpiderParser(vocab=vocab,schema_encoder=schema_encoder,
                    decoder_beam_search=beam, input_attention=DotProductAttention(),
                    past_attention=DotProductAttention(), max_decoding_steps=200)
