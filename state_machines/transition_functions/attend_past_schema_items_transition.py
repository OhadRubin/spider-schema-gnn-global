...
from allennlp_semparse.state_machines.states.grammar_based_state import (
    GrammarBasedState,
)
from state_machines.transition_functions.basic_transition_function import (
...
class AttendPastSchemaItemsTransitionFunction(BasicTransitionFunction):
    ...
    def _update_decoder_state(
        ...
        if self._num_layers > 1:
            hidden_state = torch.stack(
                [rnn_state.hidden_state for rnn_state in state.rnn_state], 1
            )
            memory_cell = torch.stack(
                [rnn_state.memory_cell for rnn_state in state.rnn_state], 1
            )
        else:
            hidden_state = torch.stack(
        ...
        for i, rnn_state in enumerate(state.rnn_state):
            if rnn_state.decoder_outputs is not None:
                ...
                attn_weights = self.attend(
                    self._past_attention,
                    hidden_state[i].unsqueeze(0),
                    decoder_outputs_states.unsqueeze(0),
                    None,
                ).squeeze(0)
                past_schema_items_attention_weights.append(
                    (attn_weights, decoder_outputs_ids)
                )
            else:
        ...
        if self._add_action_bias:
            ...
            predicted_action_embeddings = torch.cat(
                [predicted_action_embeddings, ones], dim=-1
            )
        return {
    ...
    def _compute_action_probabilities(
        ...
        batch_results: Dict[
            int, List[Tuple[int, Any, Any, Any, List[int]]]
        ] = defaultdict(list)
        for group_index in range(group_size):
            ...
            if "linked" in instance_actions:
                ...
                if past_schema_items_attention_weights[group_index] is not None:
                    (
                        past_items_attention_weights,
                        past_items_action_ids,
                    ) = past_schema_items_attention_weights[group_index]
                    past_schema_items_ids = [
                        state.action_entity_mapping[batch_id][a]
                        for a in past_items_action_ids
                    ]

                    # we are only interested about the scores of the entities the decoder has already output
                    past_entity_linking_scores = entity_action_linking_scores[
                        :, past_schema_items_ids
                    ]

                    linked_action_ent2ent_logits = past_entity_linking_scores.mm(
                        past_items_attention_weights.unsqueeze(-1)
                    ).squeeze(-1)
                    linked_action_ent2ent_logits = self._ent2ent_ff(
                        linked_action_ent2ent_logits.unsqueeze(-1)
                    ).squeeze(1)
                else:
                    linked_action_ent2ent_logits = 0

                linked_action_logits_encoder = linking_scores.mm(
                    attention_weights[group_index].unsqueeze(-1)
                ).squeeze(-1)
                linked_action_logits = (
                    linked_action_logits_encoder + linked_action_ent2ent_logits
                )
    ...
    def _construct_next_states(
        ...
        attended_question = [
            x.squeeze(0)
            for x in updated_rnn_state["attended_question"].chunk(group_size, 0)
        ]
        ...
        for _, results in batch_action_probs.items():
            ...
            if False:  # TODO: fix this
                ...
                for group_index, log_probs, _, action_embeddings, actions in results:
                    for log_prob, action_embedding, action in zip(
                        ...
                        if action in allowed_actions[group_index]:
                            new_states.append(
                                make_state(
                                    group_index, action, log_prob, action_embedding
                                )
                            )
            else:
                ...
                batch_states_idx = [
                    ...
                    if (
                        not allowed_actions
                        or group_actions[i] in allowed_actions[group_indices[i]]
                    )
                ]
