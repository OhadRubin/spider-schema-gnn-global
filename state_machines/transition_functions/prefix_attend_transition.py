...
class PrefixAttendTransitionFunction(LinkingTransitionFunction):
    def __init__(
        ...
        self._attend_output_projection_layer = Linear(
            encoder_output_dim * 2, encoder_output_dim
        )
    ...
    def _update_decoder_state(
        ...
        if not state.action_history[0]:
            attended_output, output_attention_weights = (
                self._first_attended_output.unsqueeze(0).repeat(group_size, 1),
                None,
            )
        else:
            decoder_outputs = torch.stack(
                [rnn_state.decoder_outputs for rnn_state in state.rnn_state]
            )
            decoded_item_embeddings = torch.stack(
                [rnn_state.decoded_item_embeddings for rnn_state in state.rnn_state]
            )

            action_query = torch.cat([hidden_state, attended_question], dim=-1)

            # (group_size, action_embedding_dim)
            projected_query = self._activation(
                self._attend_output_projection_layer(action_query)
            )
            query = self._dropout(projected_query)

            attended_output, output_attention_weights = self.attend(
                query, decoder_outputs, decoded_item_embeddings
            )

        # (group_size, decoder_input_dim)
        projected_input = self._input_projection_layer(
            torch.cat(
                [attended_question, previous_action_embedding + attended_output], -1
            )
        )
    ...
    def _construct_next_states(
        ...
        def make_state(
            ...
            action_entity_id = (
                state.action_entity_mapping[batch_index][action] + 1
            )  # add 1 so that -1 becomes 0 (pad)
            if not state.action_history[0]:
                decoded_item_embeddings = (
                    state.rnn_state[group_index]
                    .item_embeddings[action_entity_id]
                    .unsqueeze(0)
                )
            else:
